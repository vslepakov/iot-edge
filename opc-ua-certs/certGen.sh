...
function warn_certs_not_for_production()
{
    if [ "$FORCE_NO_PROD_WARNING" != "true" ]; then
        ...
        echo "This script is only to help you understand OPC UA CA Certificates."
...
function initial_cert_generation()
{
    ...
    convertPemToDer ${ROOT_CA_PREFIX}
    convertPemToDer ${INTERMEDIATE_CA_PREFIX}
}
...
convertPemToDer() {
    openssl x509 -outform der -in "${CERTIFICATE_DIR}/certs/${1}.cert.pem" -out "${CERTIFICATE_DIR}/certs/${1}.cert.der"
}
